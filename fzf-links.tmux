...
PYTHONPATH=\"$SCRIPT_DIR/tmux-fzf-links-python-pkg\" \"$python_path\" -m tmux_fzf_links \"$history_limit\" \"$editor_open_cmd\" \"$browser_open_cmd\" \"$fzf_display_options\" \"$path_extension\" \"$loglevel_tmux\" \"$loglevel_file\" \"$log_filename\" \"$ls_colors_filename\" \"$user_schemes_path\""
